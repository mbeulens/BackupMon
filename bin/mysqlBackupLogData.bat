...
D:\SyntecServer\mysql\bin\mysqldump -uroot -p{secret} --single-transaction=TRUE  --result-file="D:\SyntecServer\backup\mysql\backup.logdata.%DATE:~0,3%.sql" syntec  s_tab_log a_msg a_msg_ses s_usage s_usage_archive s_ses _ses_tok s_sys_log mv_s_prt mv_a_car s_mail_queue
